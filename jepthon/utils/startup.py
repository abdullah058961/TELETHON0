...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await jepiq.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph/file/c3e7a9fe3813fd062a48f.jpg",
                caption="**᯽︙ بــوت فـايـدر يـعـمـل بـنـجـاح ✓ **\n**᯽︙ ارسل `.الاوامر` لرؤية اوامر السورس**",
                buttons=[(Button.url("سورس فايدر", "https://t.me/FF_514"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
    ...
    try:
        if msg_details:
            ...
            text = (
                message.text
                + "\n\n**᯽︙اهلا وسهلا لقد قمت باعاده تشغيل بـوت فايدر تمت بنجاح**"
            )
...
async def add_bot_to_logger_group(chat_id):
    ...
    try:
        ...
        try:
            ...
            LOGS.error(str(e))
#by @FR_754 بس اشوفك خامطه للكود اهينك وافضحك 
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        descript = "- عزيزي المستخدم هذه هي مجموعه الاشعارات يرجى عدم حذفها  - @FR_754"
        photobt = await jepiq.upload_file(file="JepIQ/razan/resources/start/Jepthon.JPEG")
        _, groupid = await create_supergroup(
            "مجموعة أشعارات فايدر ", jepiq, Config.TG_BOT_USERNAME, descript, photobt
        )
