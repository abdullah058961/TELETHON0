...
def main_menu():
    text = f"**- [سـورس فـايـدر](https://t.me/FR_754)\
        \n المسـاعد\
        \n\
        \n◽ فـايـدر لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("jepiq"):
            buttons = [
                (
                    ...
                    Button.url(" Faider UsᴇʀBoᴛ", "https://t.me/FR_754"),
                )
            ]
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس فـايـدر 🧸♥",
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس فـايـدر 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس فــايـدر 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/FR_754"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/FF_514",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت فايدر 🧸♥](https://t.me/FR_754)**\
            \n\
            \n❤ فايدر هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/FF_514)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
